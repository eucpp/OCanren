...
let rec loop pairs onOK onFree = function
| 0 -> ()
| k ->
  let new_pairs =
  ...
  in
  printf "\n%!";
  loop new_pairs onOK onFree (k-1)

let run_gen onOK onFree n num handler (repr, goal) =
  make_title n repr;
  let () = try loop (MiniKanren.run num goal handler) onOK onFree n with NoMoreAnswers -> () in
  make_footer ()

let run_gen_with_env onOK onFree n num handler (repr, goal) =
  make_title n repr;
  let () = try loop (MiniKanren.run_with_env num goal handler) onOK onFree n with NoMoreAnswers -> () in
  make_footer ()
...
let run_exn_with_env printer = run_gen_with_env
  (fun i name x -> printf "%s%s=%s;%!" (if i<>0 then " " else "") name (printer x) )
  (fun _ _ _ -> failwith "Free logic variables in the answer")
